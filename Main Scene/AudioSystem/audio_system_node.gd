...
@export var song_offset: float = 0#106#107#72#23.0
...
func _physics_process(_delta):
	...
	if last_reported_beat < track_position_in_beats:
		handle_beat(track_name, sec_per_beat, last_reported_beat)
		
		last_reported_beat = track_position_in_beats


func handle_beat(track_name: String, sec_per_beat: float, beat_no: int) -> void:
	...
	var songdata_arr: Array = SongData.find_action_by_beat_no(track_name, beat_no)
